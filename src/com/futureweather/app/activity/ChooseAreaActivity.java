...
public class ChooseAreaActivity extends Activity {
	...
	private FutureWeatherDB futureWeatherDB;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		futureWeatherDB = FutureWeatherDB.getInstance(this);
	...
	private void queryProvinces() {
		provinceList = futureWeatherDB.loadProvinces();
	...
	protected void queryCities() {
		cityList = futureWeatherDB.loadCities(selectedProvince.getId());
		if(cityList.size() > 0) {
			dataList.clear();
	...
	protected void queryCounties() {
		countyList = futureWeatherDB.listCounties(selectedCity.getId());
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener() {
			...
			public void onFinish(String response) {
				...
				if("province".equals(type)) {
					result = Utility.handleProvincesResponse(futureWeatherDB, response);
				} else if("city".equals(type)) {
					result = Utility.handleCitiesResponse(futureWeatherDB,
							response, selectedProvince.getId());
				}else if("county".equals(type)) {
					result = Utility.handleCountiesResponse(futureWeatherDB,
	...
	/**
	 * 获取Back按键，根据当前级别来判断，此时应该返回市列表、省列表，还是直接退出
	 */
	@Override
